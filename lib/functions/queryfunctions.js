...
function viewAllDepartments(){
    connections.connection.query(
        "SELECT departments.name, departments.total_budget, SUM(roles.salary) AS used_budget FROM employees INNER JOIN roles on roles.id = employees.role INNER JOIN departments ON roles.department_id = departments.id GROUP BY departments.name, departments.total_budget;",
        function(err, res) {
            if (err) throw err;
            let displayTable = [["Department", "Total Budget", "Used Budget"], ["-----------", "-----------", "-----------"]];
            for (const department of res) {
                displayTable.push([department.name, department.total_budget, department.used_budget]);
            }
            console.log(" ");
            console.log(table.table(displayTable));
        }
